...
def resnet101_attack(num_classes, pretrained=True, kmax=1, kmin=None, alpha=1, num_maps=1, save_model_path=None):
    model = models.resnet101(pretrained=True)
    pooling = nn.Sequential()
    pooling.add_module('class_wise', ClassWisePool(num_maps))
    pooling.add_module('spatial', WildcatPool2d(kmax, kmin, alpha))
    model = ResNetWSL(model, num_classes * num_maps, pooling=pooling)    
