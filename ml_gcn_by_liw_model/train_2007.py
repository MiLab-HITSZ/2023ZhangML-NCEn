...
def main_gpmr_voc2007():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for model in model_list:
            save_checkpoint(model, i, is_best, best_loss,
                            save_model_path='../checkpoint/mlliw/new_gpmr_voc2007/',
...
def main_gpmr_voc2012():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for model in model_list:
            save_checkpoint(model, i, is_best, best_loss,
                            save_model_path='../checkpoint/mlliw/gpmr_voc2012/',
                            filename='voc2012_checkpoint.pth.tar')
            i += 1
...
def log_det(y_true, y_pred, model_num, model_classes, batch_size_):
        ...
        for i in range(batch_size):
            zero = torch.tensor(0, dtype=torch.float32)
            det_offset = 1e-6
            bool_R_y_true = torch.ne(torch.ones_like(y_t[i]) - y_t[i], zero)  # batch_size X (num_class X num_models), 2-D
            #print(bool_R_y_true)
            mask_non_y_pred = torch.masked_select(y_p[i], bool_R_y_true)  # batch_size X (num_class-x) X num_models, 1-D
            #print('bool_R:')
            #print(mask_non_y_pred.shape)
            mask_non_y_pred = torch.reshape(mask_non_y_pred,
                                         [1, model_num, -1])  # batch_size X num_model X (num_class-1), 3-D
            #print('bool_R:')
            #print(mask_non_y_pred.shape)
            mask_non_y_pred = mask_non_y_pred / torch.norm(mask_non_y_pred, p = 2, dim=2,
                                                        keepdim=True)  # batch_size X num_model X (num_class-1), 3-D
            ...
            all_log_det += torch.logdet(
                matrix + det_offset * torch.unsqueeze(torch.eye(model_num), 0).cuda())  # batch_size X 1, 1-D
            #print('bool_R:')
...
def main_adp_voc2012():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for model in model_list:
            save_checkpoint(model, i, is_best, best_loss,
                            save_model_path='../checkpoint/mlliw/10_adp_voc2012/',
                            filename='voc2012_checkpoint.pth.tar')
            i = i+1
