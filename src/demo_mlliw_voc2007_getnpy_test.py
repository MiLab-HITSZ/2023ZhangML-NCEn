...
from ml_gcn_by_liw_model.models_cat import resnet101_attack
...
parser.add_argument('--adv_file_path', default='../data/voc2007/files/VOC2007/classification_cat_adv.csv', type=str, metavar='N',
                    help='all image names and their labels ready to attack')
parser.add_argument('--adv_save_x', default='../adv_save/cat/voc2007/', type=str, metavar='N',
...
def gen_adv_file(model, target_type, adv_file_path):
    ...
    # value is {-1,1}
    np.save('../adv_save/cat/voc2007/y_target.npy', y_target)
    np.save('../adv_save/cat/voc2007/y.npy', y)
...
def evaluate_adv(state):
    ...
    adv_dataset = Voc2007Classification(args.data, 'cat_adv')
...
def main_cat():
    ...
    # load torch model
    '''
    model = resnet101_attack(num_classes, pretrained=True, kmax=args.k, alpha=args.alpha, num_maps=args.maps, save_model_path='../checkpoint/cat/voc2007/model_best.pth.tar')
    
    model.eval()
    '''
    model0 = resnet101_attack(num_classes, pretrained=True, kmax=args.k, alpha=args.alpha, num_maps=args.maps, save_model_path='../checkpoint/cat/new_gpmr_voc2007/0model_best.pth.tar')
    model1 = resnet101_attack(num_classes, pretrained=True, kmax=args.k, alpha=args.alpha, num_maps=args.maps, save_model_path='../checkpoint/cat/new_gpmr_voc2007/1model_best.pth.tar')
    model2 = resnet101_attack(num_classes, pretrained=True, kmax=args.k, alpha=args.alpha, num_maps=args.maps, save_model_path='../checkpoint/cat/new_gpmr_voc2007/2model_best.pth.tar')
    ...
    # value is {-1,1}
    y_target = np.load('../adv_save/cat/voc2007/y_target.npy')
    y = np.load('../adv_save/cat/voc2007/y.npy')
    ...
    evaluate_adv_list(state)
    #evaluate_model_list(model_list)
    #evaluate_adv(state)
...
def main_e():
    ...
    model0 = inceptionv3_attack(num_classes=num_classes,
                                 save_model_path='../checkpoint/mlliw/new_gpmr_voc2007/0gpmr_voc2007_checkpoint.pth.tar')
    model1 = inceptionv3_attack(num_classes=num_classes,
                                 save_model_path='../checkpoint/mlliw/new_gpmr_voc2007/1gpmr_voc2007_checkpoint.pth.tar')
    model2 = inceptionv3_attack(num_classes=num_classes,
                                 save_model_path='../checkpoint/mlliw/new_gpmr_voc2007/2gpmr_voc2007_checkpoint.pth.tar')
    model_list = [model0,model1,model2]
    for model in model_list:
      model.eval()
      if use_gpu:
