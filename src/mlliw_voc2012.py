...
from attacks import ensemble_model
...
from ml_gcn_model.voc import Voc2012Classification as gcnVoc2012Classification
...
parser.add_argument('--adv_batch_size', default=25, type=int,
...
parser.add_argument('--pixel_count', default=2000, type=int, metavar='N',
                    help='one pixel attack per attack pixel number')
parser.add_argument('--popsize', default=100, type=int, metavar='N',
                    help='one pixel attack popsize')
...
def gen_adv_file(model, target_type, adv_file_path):
    ...
    for i in range(len(pred)):
        if (y[i] == pred[i]).all() and np.sum(y[i]) >= 2 and count < 600:
            true_idx.append(i)
            count += 1
    ...
    print('len(adv_image_name_list)', len(adv_image_name_list))
    adv_labeled_data = {}
...
def evaluate_adv(state):
    ...
    # adv_dataset = gcnVoc2012Classification(args.data, 'mlgcn_adv', inp_name='../data/voc2012/voc_glove_word2vec.pkl')
    ...
    pixel_count_list = []
    pixel_map = {}
    dim_x = args.image_size
    dim_y = args.image_size
    maxiter = 100
    iteration = 0
    while iteration <= maxiter:
        pixel_count = max(int(dim_x * dim_y / 100), int((dim_x * dim_y / 100) * iteration / maxiter * 100))
        iteration = iteration + 10
        pixel_map[pixel_count] = 0
    print(pixel_map)
    ...
    with torch.no_grad():
        for batch_adv_x, batch_test_x in zip(dl1, dl2):
            ...
            pixel_count = np.sum(np.all((batch_adv_x != batch_test_x), axis=1) + 0)
    ...
    y_target = y_target[0:adv_pred.shape[0]]
    ...
    pixel_count_list = np.asarray(pixel_count_list)
    ...
    pixel_count_list = np.delete(pixel_count_list, attack_fail_idx, axis=0)
    one_pixel_count_idx = np.argwhere(pixel_count_list >= int(dim_x * dim_y / 100)).flatten()

    for i, pixel_count in enumerate(pixel_count_list.tolist()):
        maxiter = 100
        iteration = 0
        while iteration <= maxiter:
            tmp_pixel_count = max(int(dim_x * dim_y / 100), int((dim_x * dim_y / 100) * iteration / maxiter * 100))
            iteration = iteration + 10
            if pixel_count < tmp_pixel_count:
                pixel_map[tmp_pixel_count] = pixel_map[tmp_pixel_count] + 1
                break
    print(pixel_map)
    ...
    norm = np.delete(norm, one_pixel_count_idx, axis=0)
    max_r = np.delete(max_r, one_pixel_count_idx, axis=0)
    mean_r = np.delete(mean_r, one_pixel_count_idx, axis=0)
    rmsd = np.delete(rmsd, one_pixel_count_idx, axis=0)
    metrics = dict()
    y_target[y_target == -1] = 0
    metrics['ranking_loss'] = evaluate_metrics.label_ranking_loss(y_target, adv_output)
    metrics['average_precision'] = evaluate_metrics.label_ranking_average_precision_score(y_target, adv_output)
    ...
    with torch.no_grad():
        for batch_adv_x, batch_test_x in zip(dl1, dl2):
            ...
            for model in model_list:
                o += model(batch_adv_x) / 3

            adv_output.extend(o.cpu().numpy())
            batch_adv_x = batch_adv_x.cpu().numpy()
            batch_test_x = batch_test_x[0][0].cpu().numpy()
...
def main_e():
    ...
    # load torch model
    newfilename = 'voc2012_' + args.loss_type + '_checkpoint.pth.tar'

    model0 = inceptionv3_attack(num_classes=num_classes,
                                save_model_path='../checkpoint/mlliw/voc2012/ncen/' + '0' + newfilename)
    model1 = inceptionv3_attack(num_classes=num_classes,
                                save_model_path='../checkpoint/mlliw/voc2012/ncen/' + '1' + newfilename)
    model2 = inceptionv3_attack(num_classes=num_classes,
                                save_model_path='../checkpoint/mlliw/voc2012/ncen/' + '2' + newfilename)
    ...
    final_model = ensemble_model.EnsembleModel(model_list, ensemble_type='predict_mean')

    gen_adv_file(final_model, args.target_type, args.adv_file_path)
    '''
    model.eval()
    if use_gpu:
        model = model.cuda()
